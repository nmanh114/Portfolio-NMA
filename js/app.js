...
class DossierApp {
  constructor() {
    this.currentLang = 'vi';
    this.audio = window.detectiveAudio;
    this.magnifier = window.magnifyingGlassInspector;
...
if (footerBackBtn) {
  ...
  unsealCase() {
    ...
    if (this.audio) this.audio.playWaxSnap();
  ...
  returnToHero() {
    ...
    if (this.audio) this.audio.playPaperRustle();
  ...
  setLanguage(lang) {
    ...
    this.renderAllData();
    if (this.deductionChain) {
      this.deductionChain.setLanguage(lang);
    }
    if (this.canvasThreads) {
      this.canvasThreads.draw();
    }
    if (this.audio) this.audio.playTypewriter();
  }
      ...
      toolkitGrid.innerHTML = data.toolkit.map(t => `
        <div class="toolkit-physical-artifact" onclick="window.dossierApp.openToolDetail('${t.id}')" tabindex="0" role="button">
          ...
          <div class="artifact-inspect-hint">
            ...
            ${t.proofImage ? `<span class="artifact-proof-tag" title="${lang === 'vi' ? 'Có hồ sơ minh chứng' : 'Proof Attached'}">📜 ${lang === 'vi' ? 'Minh chứng' : 'Proof'}</span>` : ''}
          </div>
        </div>
      `).join('');
    }
  ...
  openToolDetail(id) {
    ...
    const proofSection = (tool.proofImage) ? `
      <div class="modal-proof-image-block">
        ...
        <div class="modal-proof-frame">
          <img src="${tool.proofImage}" 
               alt="Minh chứng ${tool.name}" 
               loading="lazy" 
               class="modal-proof-img" 
               onerror="this.closest('.modal-proof-image-block').style.display='none';">
